========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : RadCountUsbHost
========================================================================


AppWizard が生成した RadCountUsbHost には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには RadCountUsbHost を構成している各ファイルの概要説明が含まれています。
...
RadCountUsbHost.h
    ...
    みます） をインクルードし、また CRadCountUsbHostApp アプリケーションクラスを宣
    言します。

RadCountUsbHost.cpp
    このファイルは CRadCountUsbHostApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\RadCountUsbHost.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル RadCountUsbHost.rc によってインクルード
    されます。

res\RadCountUsbHost.rc2
...
RadCountUsbHostDlg.h, RadCountUsbHostDlg.cpp - ダイアログ
    これらのファイルは CRadCountUsbHostDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な RadCountUsbHost.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) RadCountUsbHost.pch 
